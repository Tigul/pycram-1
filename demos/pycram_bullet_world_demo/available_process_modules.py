...
def available_process_modules(desig):
    ...
    if robot_name == 'pr2':  # todo rmv hardcoded robot names
        ...
        if robot_type == 'simulated':
            return PepperProcessModules().initialized_simulated.available_process_modules(desig)
        else:
            return PepperProcessModules().initialized_real.available_process_modules(desig)
    else:
